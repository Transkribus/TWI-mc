...
# only for log levels (ie logging.WARN etc, which is a maybe crap)
import logging
import json
import re
...
class TranskribusSession(object):
    ...
    # Check session cache in case we don't need to bother the transkribus REST
    # service at all
    def check_cache(self, request, t_id, url, params=None):

        # If our request doesn't have a session we are probably being called
        # from a script. So no cache
        ...
        # TODO (but not here) - clear cache for certain t_ids that TWI
        # changes... eg on save (or other any triggers that update transkribus
        # data)
        if t_id in request.session and t_id is not "actions":
            request_id = t_gen_request_id(
                url, params, request.user.tsdata.userId if hasattr(
                    request.user, 'tsdata') else 1)
#            if t_id is "collections" : t_log("request_id:userid %s:%s" % (request_id,request.user.tsdata.userId), logging.WARN)
    ...
    # Set the session cache after a successful request to transkribus REST
    # service
    def set_cache_value(self, request, t_id, data, url, params=None):
        ...
        # Use the t_id as identifer for cached data, Store the url with key
        # "cache_url" (in first element if data is a list)
        request_id = t_gen_request_id(
            url, params, request.user.tsdata.userId if hasattr(
                request.user, 'tsdata') else 1)
    ...
    # use of helper functions and calling the appropriate data handler when
    # done
    def request(
            self,
            request,
        ...
        try:
            ...
            if e.response.status_code not in (401, 403, 400, 404, 500, 503):
                ...
                raise e
            # and handler_params is not None and "collId" in handler_params):
            if e.response.status_code == 400:
                if re.match(r'^.*/rest/user/register', url):
                    # t_register_handler only handles exceptions, if reg is
                    # successsful then t_user_data_handlercan be used
                    return t_register_handler(r, handler_params)

            # otherwise you get logged out...
            t_log("TRANSKRIBUS SAYS NO: %s (%s) - request: %s" %
                  (e.response.reason, e.response.status_code, url), logging.WARN)
# return
# HttpResponseRedirect(request.build_absolute_uri(settings.SERVERBASE+"/logout/?next={!s}".format(request.get_full_path())))
            return HttpResponse(
                e.response.reason,
                status=e.response.status_code)

        # Pass transkribus response to handler (NB naming convention is t_[t_id]_handler(r, handler_params)
        # handler_params are for things that we might need to pass through this
        # t_request to the handler
    ...
    def invalidate(self):
        ...
        t_log(
            "### t_invalidate response STATUS_CODE: %s" %
            (r.status_code),
            logging.WARN)
    ...
    # t_actions_info called to get lookup for action types for subsequent
    # action list calls
    ...
    # t_collection_recent called to get most recently accessed page in a given
    # collection
    ...
    # t_document_recent called to get most recently accessed page in a given
    # document
    ...
    def users_count(self, request, params=None):
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/userlist/count'
        t_id = "users_count"
        return self.request(
            request, t_id, url, params, "GET", None, {
                "collId": params.get('collId')})
    ...
    def users(self, request, params=None):
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/userlist'
        t_id = "users"
    ...
    def collection_metadata(self, request, params):
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/metadata'
        params['stats'] = 'true'
    ...
    def fulldoc(self, request, params):
        ...
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/' + str(params.get('docId')) + '/fulldoc'
        t_id = "fulldoc"
        return self.request(request, t_id, url, params)
    ...
    def document(self, request, collId, docId, nrOfTranscripts=None):
        url = settings.TRP_URL + 'collections/' + \
            collId + '/' + str(docId) + '/fulldoc'
        t_id = "document"
        params = {}
        if nrOfTranscripts is not None:
    ...
    def document_handler(self, r, params=None):
        ...
        for x in pages:
            # I'm aware this will replacce the legitimate key....
            x['key'] = x.get('pageNr')
    ...
    def page(self, request, collId, docId, page):
        ...
        url = settings.TRP_URL + 'collections/' + collId + \
            '/' + str(docId) + '/' + str(page) + '/list'
        t_id = "page"
        return self.request(request, t_id, url)
    ...
    def docStat(self, request, params):
        ...
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/' + str(params.get('docId')) + '/docStat'
        t_id = "docStat"
        return self.request(request, t_id, url)
    ...
    # returns (nr_of_transcribed_lines, nr_of_transcribed_words) for a whole
    # collection
    def collStat(self, request, params):

        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/collStat'
        t_id = "collStat"
        return self.request(request, t_id, url)
    ...
    def countDocTags(self, request, params):
        ...
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/' + str(params.get('docId')) + '/countTags'
        t_id = "countDocTags"
        return self.request(request, t_id, url, params)
    ...
    def countCollTags(self, request, params):
        ...
        url = settings.TRP_URL + 'collections/' + \
            str(params.get('collId')) + '/countTags'
        t_id = "countCollTags"
        return self.request(request, t_id, url, params)
    ...
    def page_handler(self, r, params=None):
        ...
        # TODO would prefer a pageId rather than "page" which is a the page
        # number
        for x in t_page:
            # key is used already for transcripts, however using key is handy
            # for jquery things like fancy tree...
            x['key'] = x.get('tsId')
    ...
    def transcript(self, request, transcriptId, url):
        ...
        params = {}
        return self.request(
            request, t_id, url, params, "GET", headers, {
    ...
    def transcript_xml(self, request, transcriptId, url):
        ...
        params = {}
        return self.request(
            request, t_id, url, params, headers, {
                'transcriptId': transcriptId})
    ...
    def crowdsourcing_subscribe(self, request, params):
        url = settings.TRP_URL + 'crowdsourcing/' + \
            str(params.get('collId')) + '/subscribe'
    ...
    def crowdsourcing_unsubscribe(self, request, params):
        url = settings.TRP_URL + 'crowdsourcing/' + \
            str(params.get('collId')) + '/unsubscribe'
    ...
    def save_transcript(
        ...
        params = {
            "status": "IN_PROGRESS",
            'parent': parent,
            'toolName': 'Web Interface'}
        headers = {"content-type": "application/xml"}

        url = settings.TRP_URL + 'collections/' + collId + \
            '/' + str(docId) + '/' + str(page) + '/text'
        r = self.s.post(
            url,
            verify=False,
            headers=headers,
    ...
    def download_mets_xml(self, mets_url):
        ...
        # TODO Consider returning some "document name" or such to enable the
        # user to verify the document title...?
        return r.text
    ...
    def ingest_mets_url(self, collId, mets_url):
        ...
        # , 'checkForDuplicateTitle': 'false'}# Perhaps this won't work even for testing! TODO Resolve!
        params = {'fileName': mets_url}
        r = self.s.post(url, params=params, verify=False)

        t_log(
            "Ingesting document from METS XML file URL: %s%% \r\n" %
            (mets_url))
        if (r.status_code == requests.codes.ok):
            return True
        else:
    ...
    def create_collection(self, request, collection_name):
        ...
        # clear cached collections list so that the new one will appear when
        # created
        request.session['collections'] = None
        t_id = "create_collection"
        # actucally this will need more working on if we decide to keep (need
        # handler etc)
        return self.request(
            request,
            t_id,
